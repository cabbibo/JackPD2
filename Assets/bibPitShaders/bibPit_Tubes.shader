  ...
  Shader "BibPit/Tubes" {
    ...
    _Emission ( "_Emission" , Color) = (1,1,1,1)
      ...
      _DiscardCutoff ("Di_DiscardCutoff", Range(0,3)) = 0
...
Vert v = _VertBuffer[_TriBuffer[vert.id]];
       ...
       float _NumTubesPerStem = _NumTubes / _NumStems;
        ...
        float whichStem = (floor(val * _NumStems)+ .01)/_NumStems;
        val = N_ID(v.debug.x+.1);//whichStem;



          float val2 = v.uv.x * .3 + v.debug.y * .7;

          val2 =  (val2 / _NumStems) + inStem/_NumStems;
  ...
  //float4 aVal = pow(tex2Dlod(_AudioMap , float4( v.uv.x ,0,0,0) ),10) *.1;
            vert.vertex = float4(v.pos + v.normal * aVal * .001,1);// float4(v.vertex.xyz,1.0f);
            vert.normal = v.normal;
            vert.tangent = float4(v.tangent,1);

          data.nor = v.normal;
          data.world = v.pos;
          data.lookupVal = float2( val2  +.01, val );
      ...
      void surf (Input v, inout SurfaceOutputStandard o) {
        ...
        float3 nor = UnpackNormal (tex2D (_BumpMap, v.texcoord2.xy));
         ...
         o.Metallic = _Metallic;//.9f;
          ...
          float d = 1-saturate(dot( v.nor,  normalize(_WorldSpaceCameraPos - v.world)));

float nID =  N_ID(v.debug.x+.01);
          float3 aVal = sampleAudio( v.debug.y * .1 +v.texcoordBase.x*.1 + mainCol.x * .1 + d  * .2  , nID );
         // float3 aVal2 =  sampleAudio( v.debug.y + length(mainCol) * .02, v.lookupVal.y );
          float3 cVal =  tex2D(_ColorMap,  nID+ v.texcoordBase.x*.5+d );//hsv(v.lookupVal.y* .1+ v.lookupVal.x -.4 +v.texcoordBase.x*.1, .5,1);
      
      

        o.Emission = pow( sampleAudio( v.texcoord1.x *10),2);// cVal * aVal;// * hsv( v.texcoordBase.x*.1);// hsv(v.texcoordBase.x * .1,1,1);//aVal2 * aVal * 2 * cVal;//(aVal* .8 )*cVal;//1;// pow(tex2D(_AudioMap , IN.debug.x/100 * .2 + IN.debug.y * .2 ),10) * 20;
      o.Albedo =  0;//(aVal* .8 +.2)+cVal*(aVal* .8 +.5);
      

 // o.Emission = cVal;
   //   o.Emission = hsv(v.lookupVal.y,1,1);//sin(v.debug.y);

    // o.Emission = hsv(v.debug.y*.5,1,1);
     // o.Albedo = 0;
      // o.Albedo = IN.texcoord1.x * 1 * hsv(  IN.debug.x/100 * .1-.1 + IN.debug.y * .05 , .5,1)*5* tex2D(_OSCMap, IN.debug.x/100)*pow(tex2D(_AudioMap , IN.debug.x/100 * .2 + IN.debug.y * .2 ),10) * 4*pow(tex2D(_AudioMap , IN.texcoord1.y ),3) * 4;
       // o.Emission = pow(tex2D(_AudioMap , IN.texcoord1.x * 1+ mainCol.x * .2 ),4) * hsv(  IN.debug.x/100 * .1-.1 + IN.debug.y * .05 , .5,1);
        ...
        if( length(o.Emission) < _DiscardCutoff ){
         discard;
        }
