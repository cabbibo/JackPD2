...
varyings vert (uint id : SV_VertexID){
  ...
  if( base < _Count ){
    ...
    if( alternate == 5 ){ extra = -l + u; uv = float2(0,1); }
      ...
      o.worldPos = (v.pos) + extra * _Size * length(aVal) * clamp(length(pow(a,1) * 1),0.1,.2);// * v.debug.y;
  ...
  return o;
      ...
      float4 frag (varyings v) : COLOR {
         ...
         // if( length( v.uv2 -.5) < .45 ){ discard;}
          ...
          float3 col = hsv( -((_AudioID)/11) * .2+ .2 - v.debug.y * .05 , .7,1);
