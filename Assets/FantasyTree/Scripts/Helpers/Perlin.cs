...
public static Vector3 snoise3( Vector3 x ){
  ...
  float noise1 = Noise( x );
  float noise2 = Noise( new Vector3( x.y - 23.7f , x.z + 63.2f , x.x + 135.4f ));
  float noise3 = Noise( new Vector3( x.z + 95.3f , x.x - 20.5f , x.y + 219.1f ));

  Vector3 finalNoiseVec = new Vector3( noise1 , noise2 , noise3 );
  return finalNoiseVec;

}


public static Vector3 CurlNoise( Vector3 p ){
  ...
  Vector3 dx = new Vector3( dif , 0 , 0 );
  Vector3 dy = new Vector3( 0 , dif , 0 );
  Vector3 dz = new Vector3( 0 , 0 , dif );


  //getting up and down vectors in all dimensions
  //do = down
  Vector3 doX = snoise3( p - dx );
  Vector3 upX = snoise3( p + dx );
  Vector3 doY = snoise3( p - dy );
  Vector3 upY = snoise3( p + dy );
  Vector3 doZ = snoise3( p - dz );
  Vector3 upZ = snoise3( p + dz );
  ...
  // small 'dif' is
  const float divisor = 1.0f / ( 2.0f * dif );
  return (new Vector3( finalDifX , finalDifY , finalDifZ ) * divisor ).normalized;
