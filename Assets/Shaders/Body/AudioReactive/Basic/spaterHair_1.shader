            ...
            float sdCapsule( float3 p, float3 a, float3 b, float r )
{
    ...
    return length( pa - ba*h ) - r;
            ...
            col = globalCol;
              ...
              float3 colorMapCol = tex2D(_FullColorMap , float2(v.debug.x/10000  + v.uv.x * .3, _ColorMapSection)).xyz;
               ...
               // float3 forward = mul((float3x3)unity_CameraToWorld, float3(0,0,1));
                ...
                if( 1-discardVal < 0 ){
                  //discard;
                    ...
                    float4 audio = SampleAudio( v.uv.x * .4 + ((v.debug.x/1000) % .8) ) ;
...
col =  colorMapCol * 1 * pow(audio,10) * _BrightnessMultiplier;// * audio * audio;


//col = reflectionColor;//colorMapCol;
//col = tex2D(_FullColorMap , float2(v.debug.x/10000 , _ColorMapSection)).xyz;

if( length(col) < .1 || v.uv.x > _Show){
  discard;
}
