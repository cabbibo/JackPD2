  ...
  Shader "IMMAT/Surface/ripplings2_tube" {
    Properties {
      ...
      void surf (Input v, inout SurfaceOutputStandard o) {
        ...
        float3 nor = UnpackNormal (tex2D (_BumpMap, v.texcoord2.xy));
         ...
         o.Metallic = _Metallic;//.9f;
          ...
          float3 aVal = sampleAudio( v.lookupVal.x + (length(mainCol)-1) * .1 , v.lookupVal.y );
          float3 cVal =  hsv(v.lookupVal.y* .1+ v.lookupVal.x * .1 -.2, .5,1);
        o.Emission =   (aVal* 1 +.1)*cVal;//1;// pow(tex2D(_AudioMap , IN.debug.x/100 * .2 + IN.debug.y * .2 ),10) * 20;
