   ...
   Tags { "Queue"="Transparent" "IgnoreProjector"="True" "RenderType"="Transparent" }
    ...
    Cull Off Lighting Off ZWrite Off Fog { Color (0,0,0,0) }
      ...
      struct varyings {
          ...
          float id        : TEXCOORD5;
          float3 tan       : TEXCOORD7;
      };


uniform sampler2D _AudioMap;
uniform sampler3D _AudioMap3D;
...
varyings vert (uint id : SV_VertexID){
  ...
  if( base < _Count ){
    ...
    float angle = snoise(v.pos*111);// hash( (float)base * 112 ) * 6.28;
    ...
    if( alternate == 5 ){ extra = -l + u; uv = float2(0,1); }
      ...
      o.worldPos = (v.pos) + extra * _Size * clamp(v.debug.x*v.debug.x*10,.2,10) * v.debug.y;//* clamp(length(pow(a,1) * 1),0.1,.2);// * v.debug.y;
  ...
  return o;
      ...
      float4 frag (varyings v) : COLOR {
         ...
         // if( length( v.uv2 -.5) < .45 ){ discard;}
          ...
          float4 a  = tex3D(_AudioMap3D, v.tan + float3( t.a * .01 , 0,0));

          a *= (pow( v.tan.x , 2 )*5 +.3);
          //float4 a  = tex2D(_AudioMap, float2(v.uv.x + t.a* .003 + (1-v.debug.y) * .1 ,_AudioTime));

          col = hsv( v.tan.z * 1 ,1,1)*saturate(col*clamp(100.6*a*a,.00001,1)* t.a ) * v.debug.y * v.debug.y;
          if( v.tan.z > 0.5/13){
            //col =0;
          }
          return float4(col*.1,1 );
      }
