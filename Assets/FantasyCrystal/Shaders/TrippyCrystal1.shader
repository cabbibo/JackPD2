...
float triangularNoise( float3 p ){
   ...
   p +=  tri3(p.xyz * .3+ float3(0,_Time.y * 0.1,0) ) *1.6;
   totalFog += triAdd(p.yxz * .3) * .35;
    
   p +=  tri3(p.xyz * .4 + 121 + float3(0,_Time.y * 0.1,0)) * 1;
   totalFog += triAdd(p.yxz * 1) * .25;
    
   p +=  tri3(p.xyz * .8 + 121 + float3(0,_Time.y * 0.1,0)) * 1;
...
float t3D( float3 pos ){
 ...
 // fPos += 100 * mul(unity_ObjectToWorld,float4(0,0,0,1)).xyz;
  return triangularNoise( fPos + float3(0,_Time.y * 0.001,0));
}
...
float totalSmoke = 0;
   ...
   float3 localP = mul(unity_WorldToObject, float4(_WorldSpaceCameraPos,1)).xyz;
        float3 eye = normalize(localP - v.ro.xyz);

      
  float3 rd = v.rd;
  
  rd = refract( eye , -v.nor , _IndexOfRefraction);
  ...
  float noiseDensity = saturate(length(smoke) - _NoiseSubtractor);
    ...
    rd = normalize(rd * (1-c*_StepRefractionMultiplier) + nor *  c*_StepRefractionMultiplier);
     col = .99*col + _Color* (dot(float3(0,1,0),nor)+1);// (nor * .5+.5);//lerp( lerp(_BaseColor,_CenterOrbColor * (nor * .5 + .5) , saturate(centerOrbDensity)),_NoiseColor* (nor * .5 + .5) , saturate(noiseDensity ));// saturate(dot(v.lightDir , nor)) * .1 *c;//hsv(c,.4, dT3D(p*3,float3(0,-1,0))) * c;//hsv(c * .8 + .3,1,1)*c;;// hsv(smoke,1,1) * saturate(smoke);
