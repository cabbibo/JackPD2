      ...
      float hash( float n ){
        ...
        void vert (inout appdata_full v,out Input o) {
            ...
            float3 worldNor = normalize(mul( unity_ObjectToWorld, float4(v.normal,0)).xyz);
                ...
                float aVal = length(sampleAudio( v.texcoord2.z , nID ));//tex2Dlod(_AudioMap,  float4( v.texcoord2.z , 0,0,0) ); 
                

                float worldScale = length(mul(unity_ObjectToWorld,float4(1,0,0,0)).xyz);

                worldScale *= length(aVal);
...
float fLerp = saturate(abs(-1+lerpDown(1-_AmountShown, v.texcoord2.z)));
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            c = tex2D(_ColorMap , IN.life * .3 + nID );
           // o.Albedo =  c;//tCol * c * tex2D(_AudioMap,  length( IN.uv_MainTex -.5) * .1 + IN.life ) * 30;// *  lerp(_BaseColorMultiplier, _TipColorMultiplier, IN.life);
            ...
            o.Emission = pow( c * sampleAudio( length( IN.uv_MainTex -.5) * .1 + IN.life,nID),2) * 10;
           // o.Emission =  pow(length( IN.uv_MainTex -.5),3);// tex2D(_AudioMap,  length( IN.uv -.5) * .1) * 1;
