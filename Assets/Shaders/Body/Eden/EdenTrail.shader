...
float3 generic_desaturate(float3 color, float factor)
{
	...
	return lerp(color, gray, factor);
            ...
            fixed4 frag (v2f v) : SV_Target
            {
                ...
                col = .0003;//
                col = tex2D(_FullColorMap ,float2(_ColorInfo.x + v.uv.x* _ColorInfo.z ,  _ColorInfo.y) );// * (1-v.uv.x);// * v.debug.y;
                col *= shadow;
             //   col *= _OSCValue;
            // col *= 30;
        ...
        //  col *=   (_Delay * 2 + saturate(_OSCValue * .6) * (1-_Delay));
          // col *= _OSCValue;//
        ...
        /// col *=  _OSCValue;// (_Delay * 2 + saturate(_OSCValue * .6) * (1-_Delay));
