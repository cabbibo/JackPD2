...
Vert v = _VertBuffer[_TriBuffer[vert.id]];
       ...
       float _NumTubesPerStem = _NumTubes / _NumStems;
        ...
        float val = N_ID(v.debug.x);//v.debug.x /_NumTubes;
  ...
  //float4 aVal = pow(tex2Dlod(_AudioMap , float4( v.uv.x ,0,0,0) ),10) *.1;
          ...
          data.world = v.pos;
          data.lookupVal = float2( val2 , val );
      ...
      void surf (Input v, inout SurfaceOutputStandard o) {
        ...
        float3 nor = UnpackNormal (tex2D (_BumpMap, v.texcoord2.xy));
         ...
         o.Metallic = _Metallic;//.9f;
          ...
          float3 aVal = sampleAudio( v.debug.y + v.texcoordBase.x*.3   , v.lookupVal.y );
          float3 aVal2 =  sampleAudio( v.debug.y + length(mainCol) * .02, v.lookupVal.y );
          float3 cVal =  tex2D(_ColorMap,v.lookupVal.y);// hsv(v.lookupVal.y* .1+ v.lookupVal.x -.4 +v.texcoordBase.x*.1, .5,1);
        o.Emission =   aVal2 * aVal * 2 * cVal;//(aVal* .8 )*cVal;//1;// pow(tex2D(_AudioMap , IN.debug.x/100 * .2 + IN.debug.y * .2 ),10) * 20;
