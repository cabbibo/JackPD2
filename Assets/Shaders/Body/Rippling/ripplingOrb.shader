...
CGPROGRAM
    ...
    #include "../../Chunks/SampleFullAudio.cginc"
...
float t3D( float3 pos ){
 ...
 // fPos += 100 * mul(unity_ObjectToWorld,float4(0,0,0,1)).xyz;
  return triangularNoise( fPos)* .5+  triangularNoise(pos * .1) *.5;
}
...
float totalSmoke = 0;
  ...
  float noiseDensity = saturate(length(smoke) - _NoiseSubtractor);
    ...
    centerOrbDensity = .03/pow(length(p),2);
    float currentSmoke = saturate(centerOrbDensity +noiseDensity);

    float4 aVal = sampleAudio( pow( currentSmoke,4) * .1  , _AudioID / _NumStems );
    currentSmoke = currentSmoke  +currentSmoke* length(aVal) * -.0;;
    c= currentSmoke;//* length(aVal) * 4;;   
    centerOrbDensity -= noiseDensity;
    totalSmoke += c;

    rd = normalize(rd * (1-c*_StepRefractionMultiplier) + nor *  c*_StepRefractionMultiplier);

    float vS =  float(i)/float(_NumSteps);

    float vPow =pow(d,4) * 2000;

    float4 aVal2 = sampleAudio( 1- saturate(pow( currentSmoke,5) * 1)  , _AudioID / _NumStems );
    col = .99*col + pow(currentSmoke,10) * 1 * (aVal2*50+0);//hsv( pow(currentSmoke,10) * 2,1,1);//sampleAudio(currentSmoke * .3 , _AudioID / _NumStems ) * 30*pow(currentSmoke,3);// hsv(currentSmoke,1,pow(currentSmoke,3)*10);// * currentSmoke;// * 10;// * hsv(aVal,1,1);//pow(currentSmoke ,10);//* currentSmoke* currentSmoke*currentSmoke;// centerOrbDensity*10 * totalSmoke * .1 *pow(sampleAudio( abs(.1-vPow * .05), _AudioID/_NumStems ),2) *.01* vPow;//* 2 * d * 100;//* hsv( vS * .2 , 1,1) ;//lerp( lerp(_BaseColor,_CenterOrbColor , saturate(centerOrbDensity)), _NoiseColor , saturate(noiseDensity));// saturate(dot(v.lightDir , nor)) * .1 *c;//hsv(c,.4, dT3D(p*3,float3(0,-1,0))) * c;//hsv(c * .8 + .3,1,1)*c;;// hsv(smoke,1,1) * saturate(smoke);
...
m = saturate( pow((1-m),_ReflectionSharpness)/2) * 2 * _ReflectionMultiplier;



    float vM = -1 + 2*m;

col += m* _ReflectionColor;
 if( col.r < 0 && col.g < 0 && col.b <0 ){
  discard;
 }
 //col += sampleAudio(totalSmoke * .03,_AudioID / _NumStems);//* _ReflectionColor * _ReflectionMultiplier;;
col *= hsv( totalSmoke * .004 + .75, .7,totalSmoke* .01);
 ...
 float3 reflection=-v.eye;//normalize(reflect( normalize(-v.eye) , -v.worldNor));
    ...
    //col = sampleAudio(m,_AudioID / _NumStems);
   // col = m;
