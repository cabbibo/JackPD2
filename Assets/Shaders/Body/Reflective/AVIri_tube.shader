Shader "IMMAT/Reflective/AVIridescentTube"
{
    Properties {
...
//
    ...
    float lookup = dot( worldViewDir , worldNormal );
                float3 col = skyColor * tex2D(_ColorMap,v.uv.x * .3);// + worldNormal* .2+ .3;//UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, worldNormal);///worldNormal * .5 + .5;//skyColor;//tex2D(_ColorMap, val * _ColorMapSize + _ColorMapStart ) * val;
               ...
               // col = v.debug.x *v.debug.x * v.debug.x* 3000 * col;
                ...
                float3 aCol = tex2D(_AudioMap, v.uv.x * .3 ).xyz;
