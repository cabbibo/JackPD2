...
varyings vert ( appdata vertex ){
  ...
  varyings o;
     ...
     float3 n =  vertex.normal;//_NormBuffer[id/3];
        ...
        float m = atan2( p.x , p.z );

        float3 a = tex2Dlod( _AudioMap , float4(m * .2 + .4,0,0,0)).xyz;

        
        p.xyz += n * length(a) *.0001;
...
float3 audio1 = tex2D(_AudioMap , float2(abs(v.localPos.x) ,0)).xyz * .1;
float tV = triangularNoise( v.localPos * 1.3 + float3(0,0,_Time.y * .2) )-.2;

  float3 vCol1 = tex2D(_AudioMap , float2(abs(sin(tV + .2 + v.localPos.x )),0)).xyz;
float3 frostSamp = 1-tex2D( _MainTex2 , v.localPos.xy * 1.4 + float2( tV *1*vCol1.x, tV * 2 *vCol1.x) * .2  +.5).xyz;
float totalSmoke = 0;
  ...
  float noiseDensity = saturate(length(smoke) - _NoiseSubtractor);
    ...
    float4 aCol =  tex2Dlod(_AudioMap, float4(float(i) * .05, 0, 0,0));
     col = .99*col + aCol.xyz*lerp( lerp(_BaseColor,_CenterOrbColor * (nor * .5 + .5) , saturate(centerOrbDensity)),_NoiseColor* (nor * .5 + .5) , saturate(noiseDensity ));// saturate(dot(v.lightDir , nor)) * .1 *c;//hsv(c,.4, dT3D(p*3,float3(0,-1,0))) * c;//hsv(c * .8 + .3,1,1)*c;;// hsv(smoke,1,1) * saturate(smoke);
...
float4 backgroundCol = tex2Dproj(_BackgroundTexture, refractedPos);
 ...
 col += pow((1-m),_ReflectionSharpness) * _ReflectionMultiplier * _ReflectionColor;
  ...
  col += tex2D(_AudioMap , float2(m * .4 ,0)).xyz * .1;

 float3 reflection= normalize(reflect( normalize(-v.eye) , -v.worldNor));
      half4 skyData = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, reflection,0); //UNITY_SAMPLE_TEXCUBE_LOD('cubemap', 'sample coordinate', 'map-map level')
        ...
        m = atan2( v.localPos.x , v.localPos.z );
        col = skyColor * tex2D(_AudioMap , float2(m * .2 + .4 ,0)).xyz * .1;



  float3 vCol = tex2D(_AudioMap , float2(abs(sin(tV + .2 * frostSamp.x))+ v.localPos.x ,0)).xyz;
  col = 0;
float vax = v.localPos.y+.5;
float vay = v.localPos.y+.5;
float vaz = v.localPos.y+.5;
vax -= vCol.x;
vay -= vCol.y;
vaz -= vCol.z;
  if( abs( vax) < .01 ){
...
col +=  frostSamp * .2 * (1-col);//1-tex2D( _MainTex2 , v.localPos.xy * 1 +.5).xyz;
//  col *= .1;
    //col = v.nor * .5 + .5;
    // col *= pow((1-m),5) * 60;
    // col += (v.nor * .5 + .5 ) * .4;
