    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert ( uint vid : SV_VertexID )
            {
                ...
                UNITY_TRANSFER_LIGHTING(o,o.world);
                return o;
            }
            ...
            fixed4 frag (v2f v) : SV_Target
            {
                ...
                ///in frag shader;
                float atten = UNITY_SHADOW_ATTENUATION(v,v.world);//IGHT_ATTENUATION(v);
                //m = 1-pow(-fern,.5);//*fern*fern;//pow( fern * fern, 1);
                //m = saturate( 1-m );

                //                m = (-m* atten);

                float4 p = tex2D( _PLightMap , v.uv * 5 );
                m = 1-((1-m)*atten);
                m *= 3;
                ...
                float4 s2 = tex2D( _ColorMap , float2(  sin(v.debug.x) * _ColorRandomSize + _ColorStart + v.debug.x * .2, .4) );
